...
ready(function(){
  ...
  panel.innerHTML='<div class="chat-head">'+stack()+'<div><b>'+TEAM_LABEL+'</b><span><em class="chat-live"><i></i>En ligne</em> \u00b7 ⚡ réponse dans l\'heure</span></div><button type="button" class="chat-close" aria-label="Fermer le chat">&times;</button></div>'
   ...
   +'<textarea name="message" maxlength="2000" required placeholder="Décrivez votre recrutement : poste, ville, urgence…" aria-label="Votre message"></textarea>'
   +'<p class="chat-error" hidden></p><button type="submit" class="chat-send">Envoyer \u00b7 réponse dans l\'heure ⚡</button><p class="chat-hint">Prénom, nom et e-mail nécessaires pour vous répondre.</p></form>';
  document.body.appendChild(launcher);document.body.appendChild(panel);
  var body=panel.querySelector(".chat-body"),form=panel.querySelector("form"),err=panel.querySelector(".chat-error"),ident=panel.querySelector(".chat-identity"),send=panel.querySelector(".chat-send"),hint=panel.querySelector(".chat-hint");
  ...
  function identityError(){var p=form.prenom.value.trim(),n=form.nom.value.trim(),m=form.email.value.trim();
    ...
    if(!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(m))return"Indiquez une adresse e-mail valide.";return""}
  form.addEventListener("submit",function(e){e.preventDefault();err.hidden=true;
    var ie=identityError();if(ie){err.textContent=ie;err.hidden=false;var f=form.prenom.value.trim().length<2?form.prenom:form.nom.value.trim().length<2?form.nom:form.email;ident.hidden=false;f.focus();return}
    ...
    try{localStorage.setItem(KEY,JSON.stringify({prenom:payload.prenom,nom:payload.nom,email:payload.email}))}catch(e){}
    var mine=say(msg,"from-user");form.message.value="";send.disabled=true;
    fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)})
      ...
      .then(function(x){send.disabled=false;
        if(x.ok){ident.hidden=true;hint.hidden=true;note("Message transmis ⚡ Réponse dans l'heure, à "+payload.email)}
        else{mine.remove();form.message.value=msg;err.textContent=x.j&&x.j.error||"Envoi impossible pour le moment.";err.hidden=false}})
      .catch(function(){send.disabled=false;mine.remove();form.message.value=msg;err.textContent="Connexion impossible. Écrivez-nous à contact@swipelink.fr.";err.hidden=false});
  });
});})();
